...
for i in range(80):
    image_finder = images_list_test[i][:-3]
    for k in range(80):
        label_finder = labels_list_test[k][:-3]
        if image_finder == label_finder:
            labels_list_final_test.append(labels_list_test[k])
            images_list_final_test.append(images_list_test[i])
count = 0 
for i in range(80):
    image_new = images_list_final_test[i][:-3]
    label_new = labels_list_final_test[i][:-3]
    if image_new == label_new:
        count = count +1
print(count)        
for i in range(80):
...
class Params():
    def __init__(self):
        ...
        self.image_size = 512
        self.num_class = 2  # 20 classes for training
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.conv_maxpool1 = torch.nn.MaxPool2d(kernel_size=2, stride = 2)
        self.conv_encode2 = self.contracting_block(64, 128)
        self.conv_maxpool2 = torch.nn.MaxPool2d(kernel_size=2, stride = 2)
        self.conv_encode3 = self.contracting_block(128, 256)
        self.conv_maxpool3 = torch.nn.MaxPool2d(kernel_size=2, stride = 2)
        # Bottleneck
